...
function Repository({ entries, onEntry, onRefresh }: { entries: EvolutionEntry[]; onEntry: (id: string, status: "active" | "rejected" | "disabled") => void; onRefresh: () => void }) {
  ...
  const rollback = async (version: number) => {
    ...
    try { await invoke("rollback_entry", { entryId: selected.id, version, expectedVersion: selected.version }); await openHistory({ ...selected, version: selected.version + 1 }); onRefresh() } catch (error) { setHistoryError(errorText(error)) }
  }
