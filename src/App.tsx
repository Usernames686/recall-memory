...
const emptySnapshot: Snapshot = {
  ...
  activityCount: 0,
  dirtyCount: 0,
  config: { provider: "remote", baseUrl: "", model: "", hasApiKey: false, contextMode: "guided", timeoutSeconds: 90, healthStatus: "unknown" },
...
export default function App() {
  ...
  const saveModel = async (input: { provider: ModelProvider; baseUrl: string; model: string; apiKey?: string; contextMode: ContextMode; timeoutSeconds: number }) => {
  ...
  return <div className="app-shell">
    ...
    <main className="workspace">
      ...
      <div className="page-wrap">
        ...
        {view === "agent" && <AgentConsole snapshot={snapshot} trace={liveTrace} onRun={() => void runEvolution()} onCancel={() => void cancelEvolution()} onRetry={() => void retryEvolution()} onEntry={updateEntry} onSaveMode={(agentMode) => saveEvolution({ ...snapshot.evolution, agentMode })} busy={busy} />}
...
function AgentConsole({ snapshot, trace, onRun, onCancel, onRetry, onEntry, onSaveMode, busy }: { snapshot: Snapshot; trace: AgentTraceEvent[]; onRun: () => void; onCancel: () => void; onRetry: () => void; onEntry: (id: string, status: "active" | "rejected" | "disabled") => void; onSaveMode: (mode: AgentMode) => Promise<unknown>; busy: string | null }) {
...
function SettingsPage({ snapshot, busy, onSaveEvolution, onSaveModel, onTestModel, onInstallMcp, onTestMcp }: { snapshot: Snapshot; busy: string | null; onSaveEvolution: (settings: EvolutionSettings) => Promise<unknown>; onSaveModel: (input: { provider: ModelProvider; baseUrl: string; model: string; apiKey?: string; contextMode: ContextMode; timeoutSeconds: number }) => Promise<unknown>; onTestModel: (input: { provider: ModelProvider; baseUrl: string; model: string; apiKey?: string; contextMode: ContextMode; timeoutSeconds: number }) => Promise<unknown>; onInstallMcp: () => Promise<unknown>; onTestMcp: () => Promise<unknown> }) {
  ...
  useEffect(() => { setBaseUrl(snapshot.config.baseUrl || "https://api.openai.com/v1"); setModel(snapshot.config.model); setContextMode(snapshot.config.contextMode); setProvider(snapshot.config.provider); setTimeoutSeconds(snapshot.config.timeoutSeconds) }, [snapshot.config])
  const tabs = [
  ...
  const modelInput = { provider, baseUrl, model, apiKey: apiKey || undefined, contextMode, timeoutSeconds }
  const saveModel = async () => { const result = await onSaveModel(modelInput); if (result) setApiKey("") }
